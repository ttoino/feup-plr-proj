...
To address the optimization criteria described in section \ref{section:problem-statement}, several predicates were implemented to calculate scores for different aspects of the schedule. These scores are then combined into an overall objective function that the program aims to maximize.

\subsubsection*{Worker preferences}

To implement worker preferences, as described in criterion \ref{criterion:worker-preferences}, two predicates were created: \texttt{calculate\_preference\_scores/3} and \texttt{calculate\_priority\_preference\_scores/4}.

The \texttt{calculate\_preference\_scores/3} predicate calculates scores for both daytime and nighttime shift preferences. It uses the \texttt{counts} predicate to count how many times each worker is assigned to their preferred shifts throughout the week.

The \texttt{calculate\_priority\_preference\_scores/4} predicate gives additional priority to workers assigned to night shifts, as mentioned in criterion \ref{criterion:nighttime-workers-prioritzation}. It iterates through each day and worker, adding a score when a worker with a night shift is also assigned to one of their preferred daytime shifts.

\subsubsection*{Rotated shifts distribution}

To ensure an equal distribution of rotated shifts, as described in criterion \ref{criterion:rotated-shifts-distribution}, the \texttt{calculate\_rotated\_shift\_scores/4} predicate was implemented. This predicate counts the occurrences of each rotated shift for each worker throughout the week. The resulting scores are then processed by the \texttt{score\_distances/2} predicate, which calculates the sum of the distances between each pair of scores, promoting a more equal distribution.

\subsubsection*{Overtime distribution}

To address the equal distribution of overtime, as mentioned in criterion \ref{criterion:overtime-distribution}, the \texttt{calculate\_overtime\_shift\_scores/4} predicate was created. Similar to the rotated shifts, this predicate counts the occurrences of overtime shifts for each worker throughout the week. The \texttt{score\_distances/2} predicate is then used to promote an equal distribution of overtime among workers.

\subsubsection*{Absences distribution}

To handle the distribution of absences, as described in criterion \ref{criterion:absenses-distribution}, two predicates were implemented: \texttt{calculate\_absences/3} and \texttt{score\_distances/2}.

The \texttt{calculate\_absences/3} predicate calculates the number of absences used by each worker during the week, subtracting it from their available absences. The \texttt{score\_distances/2} predicate is then used to promote an equal distribution of remaining available absences among workers.

\subsubsection*{Night shift distribution}

Although not explicitly mentioned in the criteria, the distribution of night shifts is also considered in the optimization. The \texttt{calculate\_night\_shift\_scores/3} predicate counts the number of night shifts assigned to each worker throughout the week. The \texttt{score\_distances/2} predicate is then used to promote an equal distribution of night shifts among workers.

\subsection{Objective function}

The overall objective function combines the scores from all the above criteria. It is implemented in the main \texttt{schedule/26} predicate. The function is divided into two parts:

1. A \texttt{MaxiScore} that sums the positive scores:
   - Preference scores for daytime shifts
   - Preference scores for nighttime shifts
   - Priority preference scores for workers with night shifts

2. A \texttt{MiniScore} that sums the distance scores (which are minimized):
   - Available absences distribution score
   - Rotated shifts distribution score
   - Overtime distribution score
   - Night shift distribution score

The final score is calculated as \texttt{Score \#= MaxiScore - MiniScore}. This score is then maximized using the \texttt{labeling} predicate with the \texttt{maximize(Score)} option.

By combining these various scoring mechanisms, the program aims to generate a schedule that balances all the optimization criteria, producing a solution that satisfies worker preferences while ensuring fair distribution of various shift types and absences.
